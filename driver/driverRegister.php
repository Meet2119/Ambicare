...
<body>
    <div class="main_form">
        ...
        <div class="outside_Form">
            ...
            <p class="login">Already have an account | <a class="login_a" href="driverLogin.php">Login</a></p>
            <?php
            if (isset($_POST['action'])) {
                ...
                $dQuery = "select * from driver where email='$email';";
                $dAffectedRows = mysqli_query($conn, $dQuery);
                $dRecords = mysqli_num_rows($dAffectedRows);
                if ($dRecords > 0) {
                    echo "<p class='c_Email'>This email ID is already registered</p>";
                } else {
                    $query = "insert into driver(name,contact,email,password)values('$name','$contact', '$email', '$password');";
