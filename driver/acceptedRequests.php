                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Accepted Ruquests</strong>
                            </div>
                            <div class="card-body">
                                <?php
                                require("../config/config.php");
                                if ($conn) {
                                    $query = "Select * from acceptedRequests where did= '$did';"; //creating a query
                                    ...
                                    $startTable = "<table class='table table-light'><thead><tr><th scope='col'>Sr No</th><th scope='col'>Client name</th><th scope='col'>Pickup Location</th><th scope='col'>Drop Location</th><th scope='col'>Fees</th></tr></thead><tbody>";
                                    ...
                                    if ($records > 0) {
                                        ...
                                        while ($row = mysqli_fetch_assoc($results)) {
                                            $srNo++;
                                            echo "<tr><td>{$srNo}</td><td>{$row['clientName']}</td><td>{$row['pickupL']}</td><td>{$row['dropL']}</td><td>{$row['Fees']}</td></tr>";
                                        }
                                        echo "${endTable}";
                                    } else {
